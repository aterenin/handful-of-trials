...
class InvertedPendulumBulletConfigModule:
    ENV_NAME = "MBRLCartpoleBullet-v0"
    TASK_HORIZON = 200
    NTRAIN_ITERS = 50
    ...
    def obs_cost_fn(obs):
        if isinstance(obs, np.ndarray):
            return -np.exp(-np.sum(
                np.square(InvertedPendulumBulletConfigModule._get_ee_pos(obs, are_tensors=False) - np.array([0.0, 0.6])), axis=1
            ) / (0.6 ** 2))
        else:
            return -tf.exp(-tf.reduce_sum(
                tf.square(InvertedPendulumBulletConfigModule._get_ee_pos(obs, are_tensors=True) - np.array([0.0, 0.6])), axis=1
            ) / (0.6 ** 2))
...
CONFIG_MODULE = InvertedPendulumBulletConfigModule
